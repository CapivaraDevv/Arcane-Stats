        ...
        <div>
          ...
          <div className="flex gap-1">
            {lane.champions.map((champ: string, cIdx: number) => (
              <img key={cIdx} src={getChampionImageUrl(champ)} alt={champ} title={champ} loading="lazy" className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-md object-cover bg-[#0077B6]/20 border border-white/5 hover:scale-110 cursor-pointer transition-all" />
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {lane.bestBuild.map((item: string, bIdx: number) => (
              <div key={bIdx} className="text-xs text-[#E0E0E0] px-2 py-1 bg-[#0B132B] rounded border border-white/10">{bIdx + 1}. {item}</div>
            ))}
          </div>
        </div>
