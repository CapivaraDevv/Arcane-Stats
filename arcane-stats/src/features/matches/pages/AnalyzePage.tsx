...
function gradeStyle(grade: string) {
  const g = grade[0]
  if (g === 'S') return { text: 'text-primary', border: 'border-primary/30', bg: 'bg-primary/10', accent: 'border-l-4 border-l-primary' }
  if (g === 'A') return { text: 'text-emerald-400', border: 'border-emerald-500/30', bg: 'bg-emerald-500/10', accent: 'border-l-4 border-l-emerald-500' }
  if (g === 'B') return { text: 'text-amber-400', border: 'border-amber-500/30', bg: 'bg-amber-500/10', accent: 'border-l-4 border-l-amber-500' }
  return { text: 'text-rose-400', border: 'border-rose-500/30', bg: 'bg-rose-500/10', accent: 'border-l-4 border-l-rose-500' }
}
...
function SectionHeader({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle?: string }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      ...
      <div>
        <h2 className="space-grotesk-title text-lg sm:text-xl font-bold text-foreground">{title}</h2>
  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[0, 1, 2, 3].map(i => <div key={i} className="h-28 rounded-xl bg-white/5" />)}
      </div>
...
function buildKpis(report: AnalysisReport) {
  ...
  const wDelta = winrateItem && winrateItem.old > 0
  ...
  return [
    {
      ...
      feedback: wDelta >= 0 ? `+${wDelta}% vs anterior` : `${wDelta}% vs anterior`,
    },
    {
      title: 'KDA Médio',
...
export default function AnalyzePage({ embedded = false }: { embedded?: boolean }) {
  ...
  const [filters, setFilters] = useState({ queue: 'solo', lane: 'all', period: '50' })
            ...
            <div className="flex flex-col items-center justify-center py-32 gap-6 text-center">
              ...
              <div>
                ...
                <p className="text-muted-foreground max-w-sm text-sm">
                  Configure os filtros acima e clique em <strong className="text-primary">Analisar</strong> para gerar seu relatório de coaching personalizado.
                </p>
              </div>
              <motion.button
                onClick={handleAnalyze}
            ...
            return (
              <>
                ...
                <ScrollReveal preset="up" delay={0}>
                  <div className={`bg-card-glass rounded-2xl border border-border p-5 sm:p-6 ${gs.accent}`}>
                    <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-3">
                          <Brain className="h-4 w-4 text-primary shrink-0" />
                          <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Diagnóstico Geral</span>
                        </div>
                        <p className={`text-sm text-foreground leading-relaxed ${!expandedDiag ? 'line-clamp-3' : ''}`}>
                        ...
                        >
                          ...
                          <ChevronRight className={`h-3 w-3 transition-transform ${expandedDiag ? 'rotate-90' : ''}`} />
                        </button>
                      </div>
                      <div className={`shrink-0 flex flex-col items-center justify-center rounded-2xl border ${gs.border} ${gs.bg} px-8 py-4`}>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Nota Global</span>
                        <span className={`font-display text-5xl font-black ${gs.text}`}>{report.gradeGlobal}</span>
                      </div>
                    </div>
                  </div>
                </ScrollReveal>
                    ...
                    <div className="grid md:grid-cols-2 gap-8 items-center">
                      ...
                      <div className="space-y-4">
                        {report.metricas.map((m, i) => (
                          <MetricBenchmarkRow key={m.name} metric={m} delay={i * 0.05} />
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-card-glass rounded-2xl border border-border p-5 sm:p-6">
                    ...
                    <div className="grid md:grid-cols-[1fr_260px] gap-6 items-start">
                      ...
                      <div className="space-y-3">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Recentes vs Anteriores</p>
                        {report.recentVsOld.map(item => {
                          const improved = item.recent >= item.old
                          const delta = item.old > 0 ? ((item.recent - item.old) / item.old) * 100 : 0
                          return (
                            <div key={item.label} className="flex items-center justify-between bg-white/5 rounded-xl px-4 py-3 gap-2">
                              <span className="text-xs text-muted-foreground w-20 shrink-0">{item.label}</span>
                              <div className="flex items-center gap-2 text-xs flex-wrap justify-end">
                                <span className="text-muted-foreground tabular-nums">{item.old}{item.unit}</span>
                                <span className="text-muted-foreground">→</span>
                                <span className="font-bold text-foreground tabular-nums">{item.recent}{item.unit}</span>
                                <span className={`flex items-center gap-0.5 font-bold ${improved ? 'text-emerald-400' : 'text-rose-400'}`}>
                                  {improved
                      ...
                      {report.pool.map((champ, i) => (
                        ...
                        >
                          <div className="col-span-2 flex items-center gap-3">
                            ...
                            />
                            <div>
                              <div className="text-sm font-semibold text-foreground">{champ.champion}</div>
                              <span className={`inline-block text-[10px] font-bold px-1.5 py-0.5 rounded border ${poolGrade(champ.grade)}`}>
                                {champ.grade}
                              </span>
                            </div>
                          </div>
                          <span className="text-right text-sm text-muted-foreground tabular-nums">{champ.games}</span>
                          <span className={`text-right text-sm font-semibold tabular-nums ${champ.winrate >= 50 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {champ.winrate}%
                          </span>
                          <span className="text-right text-sm text-foreground tabular-nums">{champ.kda.toFixed(1)}</span>
                        </motion.div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="bg-card-glass rounded-2xl border border-rose-500/20 p-5">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-bold text-sm text-foreground">Gargalos Críticos</h3>
                      </div>
                    ...
                    <div className="bg-card-glass rounded-2xl border border-emerald-500/20 p-5">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-bold text-sm text-foreground">Pontos Positivos</h3>
                      </div>
                      <ul className="space-y-3">
                    ...
                    <div className="bg-card-glass rounded-2xl border border-border p-5">
                      ...
                      <div className="space-y-5">
                        {report.metas.map((meta, i) => {
                          const pct = Math.min((meta.current / meta.target) * 100, 100)
                          const decimals = meta.current % 1 !== 0 ? 2 : 0
                          return (
                            <div key={i} className="space-y-2">
                              <div className="flex items-center justify-between text-sm">
                                <span className="text-foreground font-medium">{meta.metric}</span>
                                <span className="text-muted-foreground tabular-nums text-xs">
                                  {meta.current.toFixed(decimals)}{meta.unit}
                              ...
                              <p className="text-[10px] text-muted-foreground text-right">{Math.round(pct)}% da meta</p>
                            </div>
                          )
                        })}
                      </div>
                          ...
                          <SpotlightCard key={i} className="p-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="text-sm font-bold text-foreground">{rec.champion}</p>
                                <p className="text-xs text-muted-foreground leading-relaxed mt-0.5">{rec.motivo}</p>
                              </div>
                            </div>
                          </SpotlightCard>
                        ))}
